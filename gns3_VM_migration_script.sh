...
# --- CONFIGURATION ---
# Prompt for OLD_SERVER IP address at runtime
read -p "Enter the IP address of the old GNS3 server: " OLD_SERVER
...
sudo systemctl stop gns3 2>/dev/null
...
# 3. Synchronize the symbols directory (custom, from home)
echo "--> Copying /home/gns3/GNS3/symbols..."
mkdir -p /home/gns3/GNS3/symbols/
rsync -avzP --chmod=Du=rwx,Dg=rwx,Do=rx,Fu=rw,Fg=rw,Fo=r ${SSH_USER}@${OLD_SERVER}:/home/gns3/GNS3/symbols/ /home/gns3/GNS3/symbols/
...
# 5. Copy gns3_controller.conf to /opt/gns3/server/
echo "--> Copying gns3_controller.conf to /opt/gns3/server/..."
sudo mkdir -p /opt/gns3/server/
sudo cp /home/gns3/.config/GNS3/2.2/gns3_controller.conf /opt/gns3/server/gns3_controller.conf

# 6. Restore correct permissions on the new server
echo "--> Adjusting ownership and permissions for the 'gns3' user..."
sudo chown -R gns3:gns3 /opt/gns3/images
sudo chown -R gns3:gns3 /opt/gns3/server/gns3_controller.conf
sudo chown -R gns3:gns3 /home/gns3/GNS3/symbols
sudo chown -R gns3:gns3 /home/gns3/.config/GNS3

# 7. Restart the GNS3 Service
echo "--> Restarting GNS3 service..."
sudo systemctl start gns3 2>/dev/null
