    ...
    sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no "$USER@$IP" << EOF
        if [ -f /usr/local/bin/gns3_monitor.sh ]; then
            ...
            if [ "\$CURRENT_V" != "$LATEST_VERSION" ]; then
                echo "  -> UPDATE: v\$CURRENT_V naar v$LATEST_VERSION"
...
EOF
    ...
    echo "[STAP 3] Injecteren templates..."
    ...
    sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no "$USER@$IP" << 'EOF'
        ...
        if [ $T1_M -ne 0 ] || [ $T2_M -ne 0 ]; then
            ...
            if [ -s /tmp/work.conf ] && jq . /tmp/work.conf >/dev/null 2>&1; then
                ...
                sudo systemctl restart gns3 && echo "  -> [SUCCES] GNS3 herstart."
            fi
        else
            echo "  -> OK: Templates aanwezig."
        fi
