...
echo "--- GNS3 Fleet Sync v3.3: Verbose Status Reporting ---"
...
for i in {1..254}; do
    ...
    if ! sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=3 "$USER@$IP" "true" 2>/dev/null; then
        echo "[STATUS] Host onbereikbaar. Overslaan..."
        continue
    fi
    ...
    sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no "$USER@$IP" << EOF
        if [ -f /usr/local/bin/gns3_monitor.sh ]; then
            ...
            if [ "\$REMOTE_V" != "$LATEST_VERSION" ]; then
                ...
                echo "  -> GEEN UPDATE: Versie is reeds v$LATEST_VERSION."
            fi
        else
            echo "  -> OVERSLAAN: Script niet aanwezig op deze host."
        fi
EOF
    ...
    for IMG in "TCL_Firefox.qcow2" "pfsense-CE-272-preconfigured.qcow2"; do
        if ! sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no "$USER@$IP" "[ -f \"$DEST_DIR/$IMG\" ]" 2>/dev/null; then
            echo "  -> KOPIËREN: $IMG ontbreekt..."
            sshpass -p "$PASS" scp -o StrictHostKeyChecking=no "$SOURCE_DIR/$IMG" "$USER@$IP:$DEST_DIR/" && echo "  -> [OK] Bestand overgezet."
            sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no "$USER@$IP" "sudo chown gns3:gns3 \"$DEST_DIR/$IMG\"" && echo "  -> [OK] Eigenaar ingesteld op gns3."
        else
            echo "  -> AANWEZIG: $IMG is al op de host."
        fi
    done

    # --- STAP 3: GNS3_CONTROLLER.CONF BEWERKEN ---
    echo "[STAP 3] Controleren gns3_controller.conf..."
    # Eerst JSON bronbestand klaarzetten
    sshpass -p "$PASS" scp -o StrictHostKeyChecking=no << 'EOF' "$USER@$IP:/tmp/templates_to_add.json" 2>/dev/null
{
...
EOF
    ...
    sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no "$USER@$IP" << 'EOF'
        ...
        T1_MISSING=$(jq -e '.templates[] | select(.name == "Firefox-TCL")' "$CONF" >/dev/null 2>&1; echo $?)
        T2_MISSING=$(jq -e '.templates[] | select(.name == "pfSense-CE 2.7.2-Preconfigured")' "$CONF" >/dev/null 2>&1; echo $?)

        if [ $T1_MISSING -ne 0 ] || [ $T2_MISSING -ne 0 ]; then
            echo "  -> BEWERKEN: Config wijzigen..."
            cp "$CONF" "${CONF}.bak" && echo "  -> [OK] Backup gemaakt."
            cp "$CONF" /tmp/work.conf
            
            if [ $T1_MISSING -ne 0 ]; then
                jq --argjson t "$(jq .t1 $JSON_IN)" '.templates = [$t] + .templates' /tmp/work.conf > /tmp/step.json && mv /tmp/step.json /tmp/work.conf
                echo "  -> [OK] Firefox template toegevoegd aan werkbestand."
            fi
            if [ $T2_MISSING -ne 0 ]; then
                jq --argjson t "$(jq .t2 $JSON_IN)" '.templates = [$t] + .templates' /tmp/work.conf > /tmp/step.json && mv /tmp/step.json /tmp/work.conf
                echo "  -> [OK] pfSense template toegevoegd aan werkbestand."
            fi

            if [ -s /tmp/work.conf ] && jq . /tmp/work.conf >/dev/null 2>&1; then
                cp /tmp/work.conf "$CONF"
                sudo chown gns3:gns3 "$CONF" && sudo chmod 664 "$CONF"
                echo "  -> [OK] Config bestand overschreven."
                sudo systemctl restart gns3 && echo "  -> [OK] GNS3 service herstart."
            else
                echo "  -> [FOUT] Validatie mislukt. Config niet gewijzigd."
            fi
        else
            echo "  -> GEEN WIJZIGING: Beide templates zijn al aanwezig."
        fi
        rm -f "$JSON_IN" /tmp/work.conf
EOF
done
